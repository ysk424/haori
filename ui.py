...
_PARSER_FILENAME = "haori_svg_parser.py"
_JSON_FILENAME = "haori_pattern.json"
...
def _all_clothes_collections() -> list[Collection]:
    return [
        ...
        if collection.get("haori_role") == "clothes"
    ]
...
def _lock_scope_collections(properties, objects: list[Object]) -> list[Collection]:
    ...
    def add(collection: Collection | None) -> None:
        if collection is not None and collection.get("haori_role") == "clothes" and collection.name not in seen:
            collections.append(collection)
            seen.add(collection.name)

    add(properties.clothes_collection)
    for obj in objects:
        collection_name = str(obj.get("haori_collection", ""))
...
def _parser_data_dir() -> str:
    return bpy.utils.user_resource("DATAFILES", path="haori", create=True)
...
def _set_parse_status(message: str) -> None:
    if _parse_scene_name:
        ...
        if scene is not None and hasattr(scene, "haori"):
            scene.haori.parse_status = message
...
def _poll_svg_parser() -> float | None:
    ...
    try:
        ...
        if _parse_action == "UPDATE":
            ...
            if scene is not None and hasattr(scene, "haori"):
                scene.haori.clothes_collection = clothes_collection
                scene.haori.auto_lock = True
                _apply_auto_lock_all(scene.haori)
            part_count = sum(obj.get("haori_role") == "part" for obj in clothes_collection.objects)
...
class HaoriProperties(PropertyGroup):
    ...
    clothes_collection: PointerProperty(
        name="Clothes",
        description="Loaded Haori clothes collection used by GRAVITY",
        type=Collection,
    )
...
class HAORI_OT_load_svg(Operator):
    bl_idname = "haori.load_svg"
    bl_label = "Load"
    bl_description = "Parse the selected Illustrator PDF and load its Haori JSON"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        ...
        raw_path = context.scene.haori.svg_path
        if not raw_path:
...
class HAORI_OT_update_svg(Operator):
    ...
    def execute(self, context):
        ...
        if not os.path.isfile(svg_path) or Path(svg_path).suffix.lower() != ".pdf":
            ...
            return {"CANCELLED"}
        source_path = str(Path(str(collection.get("haori_source_svg", ""))).resolve())
...
def _run_gravity(operator: Operator, context, gravity_magnitude: float):
    ...
    try:
        if collection is None or collection.get("haori_role") != "clothes":
            raise KitsukeError("No loaded Haori clothes collection is selected.")
        ...
        sewing_required = bool(pending_parts) or (
            ...
            and not bool(collection.get("haori_sewing_verified", False))
        )
        if sewing_required:
...
class HAORI_OT_kitsuke_zero_gravity(Operator):
    bl_idname = "haori.kitsuke_zero_gravity"
...
class HAORI_OT_kitsuke(Operator):
    bl_idname = "haori.kitsuke"
...
class HAORI_OT_prepare_zozo(Operator):
    bl_idname = "haori.prepare_zozo"
...
class HAORI_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        actions.operator(HAORI_OT_load_svg.bl_idname, text="Load")
        actions.operator(HAORI_OT_update_svg.bl_idname, text="Update")
        gravity_actions = actions.row(align=True)
        gravity_actions.operator(HAORI_OT_kitsuke_zero_gravity.bl_idname, text="Zero GRAVITY")
        gravity_actions.operator(HAORI_OT_kitsuke.bl_idname, text="Normal GRAVITY")
        actions.operator(HAORI_OT_prepare_zozo.bl_idname, text="Prepare for ZOZO")
...
_classes = (
    ...
    HAORI_OT_load_svg,
    HAORI_OT_update_svg,
    HAORI_OT_kitsuke_zero_gravity,
    HAORI_OT_kitsuke,
    HAORI_OT_prepare_zozo,
    HAORI_PT_main,
)
...
def register():
    ...
    bpy.types.Scene.haori = PointerProperty(type=HaoriProperties)
