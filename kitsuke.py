...
"""Incremental, transient cloth simulation for the Haori Kitsuke workflow."""
...
_STATE_EPOCH_KEY = "haori_kitsuke_epoch"
_STATE_REVISION_KEY = "haori_kitsuke_revision"
_STATE_SEAMS_KEY = "haori_kitsuke_seams"
_STATE_SEAM_REST_KEY = "haori_kitsuke_seam_rest"
_STATE_MATRIX_KEY = "haori_kitsuke_matrix"
_STATE_BACKEND_KEY = "haori_kitsuke_backend"
_STATE_PARTS_KEY = "haori_kitsuke_parts"
_VELOCITY_ATTRIBUTE = "haori_kitsuke_velocity"
...
def _sewn_preview(collection: bpy.types.Collection) -> bpy.types.Object | None:
    previews = [obj for obj in collection.objects if obj.get("haori_role") == "sewn"]
    if len(previews) > 1:
...
def _cloth_topology(parts: list[_PartRange]) -> _ClothTopology:
    ...
    for part in parts:
        mesh = part.obj.data
        pattern_attribute = mesh.attributes.get("haori_pattern_position")
...
def _seam_constraints(preview: bpy.types.Object, part_ranges: list[_PartRange]) -> np.ndarray:
    names = list(preview.get("haori_source_parts", []))
...
def _seam_constraints_from_parts(collection: bpy.types.Collection, part_ranges: list[_PartRange]) -> np.ndarray:
    if not bool(collection.get("haori_sewing_verified", False)):
