...
if installed_check:
    from bl_ext.user_default import haori  # noqa: E402
    from bl_ext.user_default.haori import (  # noqa: E402
        kitsuke as kitsuke_module,
        mesh_loader,
        ui,
        haori_svg_parser,
    )
    from bl_ext.user_default.haori.mesh_loader import MeshLoadError  # noqa: E402
else:
    ...
    kitsuke_module = sys.modules[f"{haori.__name__}.kitsuke"]
    mesh_loader = sys.modules[f"{haori.__name__}.mesh_loader"]
...
if not installed_check:
    haori.register()
try:
    bpy.context.scene.haori.svg_path = str(svg_path)
    result = bpy.ops.haori.load_svg()
    assert result == {"FINISHED"}, result
    assert ui._parse_process is not None
    ui._parse_process.wait(timeout=30)
    ui._poll_svg_parser()
    status = bpy.context.scene.haori.parse_status
    assert status.startswith("Loaded CLOTHES_001: 2 part(s)"), status

    collection = bpy.data.collections["CLOTHES_001"]
    assert bpy.context.scene.haori.clothes_collection == collection
    ...
    if svg_path.suffix.lower() == ".pdf":
        labels = [obj["haori_panel_label"] for obj in parts]
        assert labels[0].startswith("OMOTE") and labels[1].startswith("URA"), labels
    assert all(obj.type == "MESH" and obj.get("haori_role") == "part" for obj in parts)
...
# sources, and creates loose spring edges in one combined verification mesh.
    ...
    assert sewn.get("haori_role") == "sewn"
    ...
    kitsuke_result = bpy.ops.haori.kitsuke()
    assert kitsuke_result == {"FINISHED"}, bpy.context.scene.haori.parse_status
    assert "gravity -Z 9.81" in bpy.context.scene.haori.parse_status
    assert "material/contact 20 iterations" in bpy.context.scene.haori.parse_status
    assert not hasattr(bpy.context.scene.haori, "kitsuke_backend")
    assert not hasattr(bpy.context.scene.haori, "kitsuke_iterations")
    ...
    assert not any(obj.get("haori_role") == "sewn" for obj in collection.objects)
...
# retains exact sewing pairs without a persistent combined preview mesh.
    assert not hasattr(bpy.context.scene.haori, "kitsuke_gravity")
    parts[0].location.x += 0.01
    parts[0].rotation_euler.z += 0.02
    second_kitsuke = bpy.ops.haori.kitsuke_zero_gravity()
    assert second_kitsuke == {"FINISHED"}, bpy.context.scene.haori.parse_status
    assert "gravity -Z 0" in bpy.context.scene.haori.parse_status
    assert not hasattr(bpy.context.scene.haori, "kitsuke_seam_pull_mm")
    assert bpy.ops.haori.kitsuke() == {"FINISHED"}
    assert "gravity -Z 9.81" in bpy.context.scene.haori.parse_status
    assert bpy.ops.haori.kitsuke_zero_gravity() == {"FINISHED"}
    assert "gravity -Z 0" in bpy.context.scene.haori.parse_status
    for _step in range(7):
        repeated_kitsuke = bpy.ops.haori.kitsuke()
        assert repeated_kitsuke == {"FINISHED"}, bpy.context.scene.haori.parse_status
    repeated_positions = [
        tuple(obj.matrix_world @ vertex.co)
        for obj in parts
        for vertex in obj.data.vertices
...
"""
    with tempfile.TemporaryDirectory() as update_directory:
        ...
        update_document = haori_svg_parser._parse_legacy_svg(update_path)
        update_collection = mesh_loader.create_clothes_mesh(bpy.context, update_document)
        bpy.context.scene.haori.clothes_collection = update_collection
        assert update_collection.name == "CLOTHES_003"
        update_parts = sorted(
            (obj for obj in update_collection.objects if obj.get("haori_role") == "part"),
            key=lambda obj: int(obj["haori_panel_index"]),
        )
        assert [obj["haori_panel_label"] for obj in update_parts] == ["FRONT01", "BACK-01"]
        assert all("haori_pattern_position" in obj.data.attributes for obj in update_parts)
        ...
        assert bool(update_collection["haori_sewing_verified"])
        assert bpy.ops.haori.kitsuke_zero_gravity() == {"FINISHED"}
        assert "gravity -Z 0" in bpy.context.scene.haori.parse_status
        ...
        larger_document = haori_svg_parser._parse_legacy_svg(update_path)
        ...
        assert bool(update_collection["haori_sewing_verified"])
        assert not any(obj.get("haori_role") == "sewn" for obj in update_collection.objects)
        assert bpy.ops.haori.kitsuke() == {"FINISHED"}
        assert "gravity -Z 9.81" in bpy.context.scene.haori.parse_status

        changed_sewing_svg = larger_svg.replace(">A</text>", ">B</text>")
        update_path.write_text(changed_sewing_svg, encoding="utf-8")
        changed_document = haori_svg_parser._parse_legacy_svg(update_path)
        sewing_changed, _vertex_count = mesh_loader.update_clothes_mesh(
            bpy.context, update_collection, changed_document
        )
        kitsuke_module.clear_kitsuke_session(update_collection)
        assert sewing_changed
        assert not bool(update_collection["haori_sewing_verified"])
        assert bpy.ops.haori.kitsuke() == {"FINISHED"}
        assert bool(update_collection["haori_sewing_verified"])
        ...
        wrong_labels_document = haori_svg_parser._parse_legacy_svg(update_path)
        try:
