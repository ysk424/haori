...
parser = importlib.import_module(f"{haori.__name__}.haori_svg_parser")
...
try:
    with tempfile.TemporaryDirectory() as directory:
        ...
        bpy.context.scene.haori.body_object = bpy.context.object

        parts[0].location.y += 0.01
        bpy.context.view_layer.update()
        try:
            rejected = bpy.ops.haori.kitsuke_zero_gravity()
        ...
        bpy.context.scene.haori.lock_selection = True
        ...
        bpy.context.scene.haori.auto_lock = True
        ...
        print("HAORI_GRAVITY_STATE_LOCK_OK stages=2,3,4 repeat=ok auto=on/off lock=ok")
finally:
    haori.unregister()
