...
from haori import kitsuke  # noqa: E402
from haori.zozo_handoff import (  # noqa: E402
    ZOZO_MCP_PORT,
...
def add_pattern_attribute(obj):
    attribute = obj.data.attributes.new(
        name="haori_pattern_position", type="FLOAT_VECTOR", domain="POINT"
    )
...
assert cloth.data.attributes.get("haori_source_part") is not None
stitch = cloth.data.attributes.get("haori_zozo_stitch")
...
owned = [
    ...
    if obj.get("haori_role") in {"zozo_cloth", "zozo_body"}
]
assert len(owned) == 2
