...
"""Create a non-destructive Haori hand-off for ZOZO Contact Solver."""
...
class ZozoHandoffError(RuntimeError):
    """The current Haori state cannot safely be handed to ZOZO."""
...
def _pattern_positions(obj: bpy.types.Object) -> list[tuple[float, float]]:
    ...
    if (
        ...
        raise ZozoHandoffError(
            f"{obj.name} has no valid Haori pattern coordinates; load the pattern again."
        )
...
def _handoff_collection(context, source: bpy.types.Collection) -> bpy.types.Collection:
    matches = [
        ...
        if collection.get("haori_role") == _HANDOFF_COLLECTION_ROLE
        and collection.get("haori_source_collection") == source.name
    ]
    ...
    handoff["haori_role"] = _HANDOFF_COLLECTION_ROLE
    handoff["haori_source_collection"] = source.name
    for collection in matches:
        for obj in list(collection.objects):
            if (
                obj.get("haori_source_collection") == source.name
                and obj.get("haori_role") in {_HANDOFF_CLOTH_ROLE, _HANDOFF_BODY_ROLE}
            ):
...
def _create_cloth_object(
    ...
    for part_index, obj in enumerate(parts):
        ...
        panel_index = int(obj.get("haori_panel_index", part_index))
    ...
    try:
        ...
        for item, value in zip(panel_attribute.data, face_panel_indices):
            item.value = value
        part_attribute = mesh.attributes.new(name="haori_source_part", type="INT", domain="POINT")
        for item, value in zip(part_attribute.data, vertex_part_indices):
            item.value = value
        stitch_attribute = mesh.attributes.new(name="haori_zozo_stitch", type="BOOLEAN", domain="EDGE")
        found_stitches = 0
        for edge in mesh.edges:
        ...
        uv_layer = mesh.uv_layers.new(name="Haori Pattern", do_init=False)
        ...
        cloth["haori_schema"] = "haori-pattern/1.0.0"
        cloth["haori_role"] = _HANDOFF_CLOTH_ROLE
        cloth["haori_source_collection"] = source.name
        cloth["haori_source_parts"] = [part.name for part in parts]
        cloth["haori_zozo_contact_gap_m"] = ZOZO_CONTACT_GAP_M
        cloth["haori_zozo_stitch_opening_m"] = ZOZO_STITCH_OPENING_M
        return cloth
    except Exception:
...
def _create_body_object(
    ...
    duplicate["haori_role"] = _HANDOFF_BODY_ROLE
    duplicate["haori_source_collection"] = source.name
    duplicate["haori_source_body"] = body.name
...
def _project_name(collection_name: str) -> str:
    ...
    return f"haori_{value or 'clothes'}"
...
def prepare_for_zozo(
    ...
    """Create solver-owned cloth/body copies and leave Haori untouched."""
    if collection is None or collection.get("haori_role") != "clothes":
        raise ZozoHandoffError("Select a loaded Haori Clothes collection first.")
    ...
    cloth_group_name = f"Haori {collection.name} Cloth"
    body_group_name = f"Haori {collection.name} Body"
    cloth["haori_zozo_group"] = cloth_group_name
    body_copy["haori_zozo_group"] = body_group_name
