...
"""Create an initial cloth-ready Blender mesh from Haori pattern JSON."""
...
GRAINLINE_EDGE_FAMILY_ATTRIBUTE = "haori_grainline_family"
GRAINLINE_FACE_QUAD_ATTRIBUTE = "haori_grainline_quad"
GRAINLINE_EDGE_PROXY = 0
GRAINLINE_EDGE_WARP = 1
GRAINLINE_EDGE_WEFT = 2
GRAINLINE_EDGE_TRANSITION = 3
LOAD_MATRIX_KEY = "haori_load_matrix"
LOCKED_OBJECT_KEY = "haori_kitsuke_locked"
GRAVITY_STATE_KEY = "haori_gravity_state"
...
def mark_moved_parts_pending(collection: bpy.types.Collection | None) -> tuple[bpy.types.Object, ...]:
    ...
    for obj in collection.objects:
        if obj.type != "MESH" or obj.get("haori_role") != "part":
            continue
        ...
        if state == GRAVITY_STATE_PENDING:
            ...
            pending.append(obj)
    return tuple(sorted(pending, key=lambda obj: int(obj.get("haori_panel_index", 0))))
...
def participating_parts(collection: bpy.types.Collection) -> tuple[bpy.types.Object, ...]:
    ...
    return tuple(sorted(
        (
            ...
            if (
                obj.type == "MESH"
                and obj.get("haori_role") == "part"
                and part_gravity_state(obj) != GRAVITY_STATE_PLACED
            )
        ),
        key=lambda obj: int(obj.get("haori_panel_index", 0)),
    ))
...
def _write_panel_mesh_attributes(
    ...
    rest_attribute = mesh.attributes.new(name="haori_pattern_edge_rest", type="FLOAT", domain="EDGE")
    ...
    pattern_attribute = mesh.attributes.new(name="haori_pattern_position", type="FLOAT_VECTOR", domain="POINT")
    ...
    construction_attribute = mesh.attributes.new(
        name="haori_construction_position", type="FLOAT_VECTOR", domain="POINT"
    )
...
def _sewing_signature(document: dict[str, Any]) -> str:
    ...
    if not isinstance(groups, dict):
        raise MeshLoadError("Haori JSON has no sewing_groups object.")
    ...
    for label, references in groups.items():
        if not isinstance(label, str) or not isinstance(references, list):
            raise MeshLoadError("Haori JSON has an invalid sewing group.")
        values: list[tuple[str, int]] = []
        for reference in references:
            if not isinstance(reference, dict):
                raise MeshLoadError("Haori JSON has an invalid sewing reference.")
            try:
                values.append((str(reference["panel"]), int(reference["segment"])))
            except (KeyError, TypeError, ValueError) as exc:
                raise MeshLoadError("Haori JSON has an invalid sewing reference.") from exc
        normalized[label.upper()] = sorted(values)
    panels = document.get("panels")
    if not isinstance(panels, list):
        raise MeshLoadError("Haori JSON has no panels array.")
    construction: list[dict[str, object]] = []
    for panel in panels:
        if not isinstance(panel, dict):
            raise MeshLoadError("Haori JSON contains an invalid panel.")
        segments = panel.get("segments")
        if not isinstance(segments, list):
            raise MeshLoadError("Haori JSON contains an invalid panel segment array.")
        construction.append({
            "id": str(panel.get("id", "")),
...
def create_clothes_mesh(context, document: dict[str, Any]) -> bpy.types.Collection:
    ...
    if document.get("schema") != "haori-pattern" or document.get("version") != "1.0.0":
        raise MeshLoadError("Unsupported Haori JSON schema.")
    if document.get("units") != "m":
        raise MeshLoadError("Haori mesh loading requires meter units.")
    source = document.get("source")
    panels_json = document.get("panels")
    if not isinstance(source, dict) or not isinstance(panels_json, list) or not panels_json:
        raise MeshLoadError("Haori JSON has no valid source or panels.")
    ...
    try:
        ...
        for panel_index, panel in enumerate(panels):
            ...
            obj["haori_schema"] = "haori-pattern/1.0.0"
            obj["haori_role"] = "part"
            obj["haori_collection"] = name
            obj["haori_source_svg"] = str(source.get("svg_path", ""))
            obj["haori_mesh_spacing_m"] = MESH_SPACING_M
            obj["haori_panel_id"] = panel.panel_id
            obj["haori_panel_label"] = panel.update_label or ""
            obj["haori_panel_instance"] = panel.instance_id
            obj["haori_panel_index"] = panel_index
            obj["haori_mirror_side"] = panel.mirror_side
            obj["haori_ring_closed"] = panel.ring_closed

        collection["haori_schema"] = "haori-pattern/1.0.0"
        collection["haori_role"] = "clothes"
        collection["haori_source_svg"] = str(source.get("svg_path", ""))
        collection["haori_sewing_signature"] = _sewing_signature(document)
        collection["haori_sewing_verified"] = False
...
def _pattern_positions(obj: bpy.types.Object) -> list[Vector]:
    attribute = obj.data.attributes.get("haori_pattern_position")
    if attribute is None or attribute.domain != "POINT" or len(attribute.data) != len(obj.data.vertices):
        raise UpdateError(
            f"{obj.name} has no original pattern coordinates. Load it again with the current Haori version."
        )
...
def _construction_positions(obj: bpy.types.Object) -> list[Vector]:
    attribute = obj.data.attributes.get("haori_construction_position")
    if attribute is None or attribute.domain != "POINT" or len(attribute.data) != len(obj.data.vertices):
        raise UpdateError(
            f"{obj.name} has no construction coordinates. Load it again with the current Haori version."
        )
...
def _transfer_deformation(obj: bpy.types.Object, panel: PanelGeometry) -> list[Vector]:
    ...
    if panel.ring_closed:
        if not bool(obj.get("haori_ring_closed", False)):
        ...
        return transferred
    if bool(obj.get("haori_ring_closed", False)):
...
def remove_sewn_preview(collection: bpy.types.Collection, reveal_parts: bool = False) -> None:
    ...
    if reveal_parts:
        for obj in collection.objects:
            if obj.type == "MESH" and obj.get("haori_role") == "part":
                obj.hide_set(False)
...
def update_clothes_mesh(context, collection: bpy.types.Collection, document: dict[str, Any]) -> tuple[bool, int]:
    ...
    if collection is None or collection.get("haori_role") != "clothes":
        raise UpdateError("No loaded Haori clothes collection is selected.")
    source = document.get("source")
    panels_json = document.get("panels")
    if not isinstance(source, dict) or not isinstance(panels_json, list) or not panels_json:
        raise UpdateError("Updated Haori JSON has no valid source or panels.")
    old_source = str(collection.get("haori_source_svg", ""))
    ...
    parts = sorted(
        (obj for obj in collection.objects if obj.type == "MESH" and obj.get("haori_role") == "part"),
        key=lambda obj: int(obj.get("haori_panel_index", 0)),
    )
    if not all(isinstance(panel, dict) for panel in panels_json):
        raise UpdateError("Updated Haori JSON contains an invalid panel.")
    panels = _panel_geometries(panels_json, MESH_SPACING_M)
    ...
    for obj in parts:
        label = str(obj.get("haori_panel_label", ""))
        if not label:
            raise UpdateError(f"{obj.name} has no # panel label. Load the labeled pattern again first.")
        instance_id = str(obj.get("haori_panel_instance", label))
    ...
    try:
        for instance_id, obj in old_by_instance.items():
            ...
            mesh.validate(verbose=False, clean_customdata=False)
            mesh.update(calc_edges=True, calc_edges_loose=True)
            _write_panel_mesh_attributes(mesh, panel, int(obj.get("haori_panel_index", 0)))
    ...
    for obj, mesh, panel in prepared:
        ...
        obj.data = mesh
        obj["haori_source_svg"] = new_source
        obj["haori_mesh_spacing_m"] = MESH_SPACING_M
        obj["haori_panel_id"] = panel.panel_id
        obj["haori_panel_label"] = panel.update_label
        obj["haori_panel_instance"] = panel.instance_id
        obj["haori_mirror_side"] = panel.mirror_side
        obj["haori_ring_closed"] = panel.ring_closed
        obj.hide_set(False)
    ...
    old_signature = str(collection.get("haori_sewing_signature", ""))
    new_signature = _sewing_signature(document)
    sewing_changed = old_signature != new_signature
    collection["haori_source_svg"] = new_source
    collection["haori_sewing_signature"] = new_signature
    if sewing_changed:
        collection["haori_sewing_verified"] = False
    _store_document(collection, document)
    context.view_layer.update()
...
def _seam_chains(obj: bpy.types.Object, label: str) -> list[_SeamChain]:
    ...
    adjacency: dict[int, set[int]] = {}
    rest_attribute = mesh.attributes.get("haori_pattern_edge_rest")
    valid_rest = (
...
def _multipart_closed_pairs(
    ...
    count = len(closed)
    first_obj, second_obj = sorted(open_by_object, key=lambda obj: int(obj.get("haori_panel_index", 0)))
...
def build_sewing_plan(collection: bpy.types.Collection) -> SewingPlan:
    ...
    if collection is None or collection.get("haori_role") != "clothes":
        raise SewingError("No loaded Haori clothes collection is selected.")
        ...
        try:
            if active_has_closed:
                ...
                first_obj, second_obj = sorted(
                    by_object, key=lambda obj: int(obj.get("haori_panel_index", 0))
                )
                pairs = [
...
def compute_seam_count_overrides(
    ...
    def _panel_id(obj: bpy.types.Object) -> str:
        return str(obj.get("haori_panel_id", obj.name))
...
_DOCUMENT_PROPERTY = "haori_document_json"
...
def remesh_with_seam_counts(
    ...
    old_by_instance = {
        str(obj.get("haori_panel_instance", obj.get("haori_panel_label", ""))): obj
        for obj in parts
    }
...
def create_sewn_mesh(
    ...
    if collection is None or collection.get("haori_role") != "clothes":
        raise SewingError("No loaded Haori clothes collection is selected.")
    if any(obj.get("haori_role") == "sewn" for obj in collection.objects):
    ...
    for obj in parts:
        ...
        face_panel_indices.extend([int(obj.get("haori_panel_index", 0))] * len(mesh.polygons))
    ...
    try:
        ...
        obj["haori_schema"] = "haori-pattern/1.0.0"
        obj["haori_role"] = "sewn"
        obj["haori_collection"] = collection.name
        obj["haori_source_svg"] = str(collection.get("haori_source_svg", ""))
        obj["haori_sewing_groups"] = labels
        obj["haori_source_parts"] = [part.name for part in parts]
        collection["haori_sewing_verified"] = True
