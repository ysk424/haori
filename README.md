...
v0.1.0はBlender 5.2 LTSで検証しています。配布ZIPにはネイティブソルバーとMicrosoft OpenMP Runtimeが含まれます。

## インストール

1. [Releases](https://github.com/ysk424/haori/releases)から`haori-0.1.0-windows_x64.zip`を取得します。
...
各中間姿勢を実際に評価し、隣接姿勢間の移動が`L`を超えた場合は分割数を増やします。各姿勢につきNormal Gravity 1回相当を計算します。内部では8サブステップ、20反復を使用します。
...
同じYohsai Clothesで再実行すると、以前のHAORI出力は新しい計算開始時に削除されます。
...
- 再実行開始時に以前のHAORIキャッシュを置換するため、再計算前に必要な結果を別名で保存する必要がある
- v0.1.0のHAORIソルバーは明示的な実行時並列化を行わず、Body分割を順番に計算する
- v0.1.0の配布バイナリはWindows x64のみ
...
macOS/Linux用のビルドスクリプトもありますが、v0.1.0のマニフェストと配布物はWindows x64のみを対象にしています。
