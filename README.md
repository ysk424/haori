...
v1.0.0はBlender 5.2 LTSで検証しています。配布ZIPにはネイティブソルバーとMicrosoft OpenMP Runtimeが含まれます。

## インストール

1. [Releases](https://github.com/ysk424/haori/releases)から`haori-1.0.0-windows_x64.zip`を取得します。
...
| Standard | 1.0cm | 1.0cm | 20 | Body移動距離と接触余裕を揃えた標準プレビュー |
...
- v1.0.0のHAORIソルバーは明示的な実行時並列化を行わず、Body分割を順番に計算する
- v1.0.0の配布バイナリはWindows x64のみ
...
macOS/Linux用のビルドスクリプトもありますが、v1.0.0のマニフェストと配布物はWindows x64のみを対象にしています。
