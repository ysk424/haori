...
"""Standalone Illustrator PDF to Haori JSON converter.
...
OUTPUT_FILENAME = "haori_pattern.json"
TEMP_FILENAME = OUTPUT_FILENAME + ".tmp"
SCHEMA_NAME = "haori-pattern"
...
class ParseError(ValueError):
    """A PDF does not satisfy the Haori input profile."""
...
class Point:
    ...
    def as_json(self, scale: float) -> list[float]:
        # Internal source points use a downward Y convention; Haori JSON uses up.
...
def _pdf_collect_content(
    ...
    for operands, operator in stream.operations:
        if operator == b"q":
            ...
            # sublayer names never select or exclude Haori pattern content.
            continue
        elif operator == b"Do":
...
def parse_pdf(path: str | os.PathLike[str]) -> dict[str, object]:
    ...
    if len(reader.pages) != 1:
        raise ParseError(f"Haori PDF input must contain exactly one page; found {len(reader.pages)}.")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("Usage: haori_svg_parser.py <absolute-pattern.pdf>", file=sys.stderr)
        return 2
    try:
        document = parse_pattern(arguments[0])
        output_path = write_fixed_output(document)
    except (ParseError, OSError, ValueError) as exc:
        print(f"Haori pattern parse failed: {exc}", file=sys.stderr)
        return 1
    print(output_path)
    return 0
